...
// signal-cli is a commandline interface for libsignal-service-java. It supports registering, verifying,
// sending and receiving messages. see signal-cli Manual Page - https://github.com/AsamK/signal-cli/blob/master/man/signal-cli.1.adoc
...
impl Manager {
    ...
    /// * `account` - Specify your phone number, that will be your identifier. The phone number must include
    ///   the country calling code, i.e. the number must start with a "+" sign.
    pub fn new(account: Account, trust_mode: TrustMode) -> Manager {
        let config = Config::new(account.host().clone(), account.service_environment().clone());
        Manager {
            account,
            config,
            trust_mode,
    ...
    /// Register a phone number with SMS or voice verification. Use the verify command to complete the
    /// verification.
    ///
    /// For registering you need a phone number where you can receive SMS or incoming calls.
    ///
    /// If the account is just deactivated, the register command will just reactivate account, without
    /// requiring an SMS verification. By default the unregister command just deactivates the account, in
    /// which case it can be reactivated without sms verification if the local data is still available. If the
    /// account was deleted (with --delete-account) it cannot be reactivated.
    ///
    /// # Arguments
    /// * `voice` - The verification should be done over voice, not SMS.
    /// * `captcha` - The captcha token, required if registration failed with a captcha required error. To get
    ///   the token, go to https://signalcaptchas.org/registration/generate.html For the staging environment, use:
    ///   https://signalcaptchas.org/staging/registration/generate.html Check the developer tools for a
    ///   redirect starting with signalcaptcha:// Everything after signalcaptcha:// is the captcha token.
    #[allow(dead_code)]
    ...
    /// Disable push support for this device, i.e. this device won’t receive any more messages. If this is the
    /// primary device, other users can’t send messages to this number anymore. Use "updateAccount" to undo
    /// this. To remove a linked device, use "removeDevice" from the primary device.
    ///
    /// # Arguments
    /// * `delete_account` - Delete account completely from server. Cannot be undone without loss. You will
    ...
    /// * `unidentified_delivery_indicators` - Indicates if Signal should show unidentified delivery
    ///   indicators.
    ...
    ///
    /// # Arguments
    /// * `registration_lock_pin` - The registration lock PIN, that will be required for new registrations
    ...
    /// "sgnl://linkdevice?uuid=…​" URI. If you want to connect to another signal-cli instance, you can just
    /// use this URI. If you want to link to an Android/iOS device, create a QR code with the URI (e.g. with
    ...
    ///
    /// # Arguments
    /// `uri` - Specify the uri contained in the QR code shown by the new device. You will need the full URI
    /// such as "sgnl://linkdevice?uuid=…​" (formerly "tsdevice:/?uuid=…​") Make sure to enclose it in quotation
    /// marks for shells.
    #[allow(dead_code)]
    ...
    /// Uses a list of phone numbers to determine the statuses of those users. Shows if they are registered on
    /// the Signal Servers or not. In json mode this is outputted as a list of objects.
    ///
    /// # Arguments
    ...
    /// * `attachments` -Add one or more files as attachment. Can be either a file path or a data URI. Data
    ///   URI encoded attachments must follow the RFC 2397. Additionally a file name can be added: e.g.:
    ///   data:<MIME-TYPE>;filename=<FILENAME>;base64,<BASE64 ENCODED DATA>
    /// * `sticker` - Send a sticker of a locally known sticker pack (syntax: stickerPackId:stickerId).
    ///   Shouldn’t be used together with -m as the official clients don’t support this. e.g.: --sticker
    ///   00abac3bc18d7f599bff2325dc306d43:2
    /// * `mention` - Mention another group member (syntax: start:length:recipientNumber) In the apps the
    ///   mention replaces part of the message text, which is specified by the start and length values. e.g.:
    ///   -m "Hi X!" --mention "3:1:+123456789"
    /// * `text_style` - Style parts of the message text (syntax: start:length:STYLE). Where STYLE is one of:
    ///   BOLD, ITALIC, SPOILER, STRIKETHROUGH, MONOSPACE
    /// * `quote_timestamp` - Specify the timestamp of a previous message with the recipient or group to add a
    ...
    /// * `preview_url` - Specify the url for the link preview. The same url must also appear in the message
    ///   body, otherwise the preview won’t be displayed by the apps.
    ...
    /// * `edit_timestamp` - Specify the timestamp of a previous message with the recipient or group to send
    ///   an edited message.
    #[allow(dead_code)]
    pub fn send(
        _recipient: Vec<&str>,
    ...
    #[allow(dead_code)]
    pub fn send_payment_notification(_recipient: &str, _receipt: &str, _note: &str) -> Result<(), Error> {
        todo!();
    }
    ...
    // Send typing message to trigger a typing indicator for the recipient. Indicator will be shown for
    // 15seconds unless a typing STOP message is sent first.
    ///
    /// # Arguments
    /// * `recipient` - Specify the sender’s phone number.
    /// * `group_id` - Specify the recipient group ID in base64 encoding.
    ...
    #[allow(dead_code)]
    pub fn send_typing(_recipient: Vec<&str>, _group_id: Vec<&str>, _stop: bool) -> Result<(), Error> {
        todo!();
    }
    ...
    /// Query the server for new messages. New messages are printed on standard output and attachments are
    /// downloaded to the config directory. In json mode this is outputted as one json object per line.
    ///
    /// # Arguments
    /// * `timeout` - Number of seconds to wait for new messages (negative values disable timeout). Default is
    ///   5 seconds.
    ...
    /// * `send-read-receipts` - Send read receipts for all incoming data messages (in addition to the default
    ///   delivery receipts)
    #[allow(dead_code)]
    pub fn receive(
        _timeout: f64,
    ...
    /// Create or update a group. If the user is a pending member, this command will accept the group
    /// invitation.
    ///
    /// # Arguments
    /// * `group_id' - Specify the recipient group ID in base64 encoding. If not specified, a new group with a
    ...
    /// * `ban' - Specify one or more members to ban from joining the group. Banned members cannot join or
    ///   request to join via a group link.
    ...
    /// * `set_permission_send_messages' - Set permission to send messages in group: every-member, only-admins
    ///   Groups where only admins can send messages are also called announcement groups
    /// * `expiration' - Set expiration time of messages (seconds). To disable expiration set expiration time
    ///   to 0.
    #[allow(dead_code)]
    pub fn update_group(
    ...
    /// Send a quit group message to all group members and remove self from member list. If the user is a
    /// pending member, this command will decline the group invitation.
    ///
    /// # Aurguments
    /// * `group_id` - Specify the recipient group ID in base64 encoding.
    ...
    // Show a list of known groups and related information. In json mode this is outputted as an list of
    // objects and is always in detailed mode.
    ///
    /// # Arguments
    ...
    // Show a list of known contacts with names and profiles. When a specific recipient is given, its profile
    // will be refreshed.
    ///
    /// # Arguments
    /// * `recipient` - Specify the recipients’ phone number.
    ...
    /// Set the trust level of a given number. The first time a key for a number is seen, it is trusted by
    /// default (TOFU). If the key changes, the new key must be trusted manually.
    ///
    /// # Arguments
    ...
    /// * `verified_safety_number` - Specify the safety number of the key, only use this option if you have
    ///   verified the safety number. Can be either the plain text numbers shown in the app or the bytes from
    ///   the QR-code, encoded as base64.
    #[allow(dead_code)]
    pub fn trust(
        _recipient: &str,
    ...
    // Update the profile information shown to message recipients. The profile is stored encrypted on the
    // Signal servers. The decryption key is sent with every outgoing messages to contacts and included in
    // every group.
    ///
    /// # Arguments
    ...
    /// Update the info associated to a number on our contact list. This change is only local but can be
    /// synchronized to other devices by using sendContacts (see below). If the contact doesn’t exist yet, it
    /// will be added.
    ///
    /// # Arguments
    ...
    /// * `block` - Block the given contacts or groups (no messages will be received). This change is only
    ///   local but can be synchronized to other devices by using sendContacts (see below).
    ...
    // Unblock the given contacts or groups (messages will be received again). This change is only local but
    // can be synchronized to other devices by using sendContacts (see below).
    ///
    /// # Arguments
    ...
    /// Send a synchronization message with the local contacts list to all linked devices. This command should
    /// only be used if this is the primary device.
    #[allow(dead_code)]
    pub fn send_contacts() -> Result<(), Error> {
        todo!();
    }

    /// Send a synchronization request message to the primary device (for group, contacts, …​). The primary
    /// device will respond with synchronization messages with full contact and group lists.
    #[allow(dead_code)]
    ...
    /// Gets the raw data for a specified attachment. This is done using the ID of the attachment the
    /// recipient or group ID. The attachment data is returned as a Base64 String.
    ///
    /// # Arguments
    ...
    #[allow(dead_code)]
    pub fn trust_mode_set(&mut self, trust_mode: TrustMode) { self.trust_mode = trust_mode; }
