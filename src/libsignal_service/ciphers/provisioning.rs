...
use crate::libsignal_service::envelope::{CIPHER_KEY_SIZE, IV_LENGTH, IV_OFFSET};
use crate::libsignal_service::errors::ServiceError;
use crate::signalservice::{ProvisionEnvelope, ProvisionMessage};
...
impl Debug for CipherMode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        match self {
            CipherMode::DecryptAndEncrypt(key_pair) => {
                f.debug_tuple("CipherMode::DecryptAndEncrypt").field(&key_pair.public_key).finish()
            }
            CipherMode::EncryptOnly(public) => {
                f.debug_tuple("CipherMode::EncryptOnly").field(&public).finish()
            }
        }
    }
}
...
impl ProvisioningCipher {
        ...
        R: rand::Rng + rand::CryptoRng,
    {
        ...
        Ok(Self { key_material: CipherMode::DecryptAndEncrypt(key_pair) })
    }

    pub fn from_public(key: PublicKey) -> Self { Self { key_material: CipherMode::EncryptOnly(key) } }

    pub fn from_key_pair(key_pair: KeyPair) -> Self {
        Self { key_material: CipherMode::DecryptAndEncrypt(key_pair) }
    }

    pub fn public_key(&self) -> &PublicKey { self.key_material.public() }
    ...
    pub fn encrypt(&self, msg: ProvisionMessage) -> Result<ProvisionEnvelope, ProvisioningError> {
        ...
        let mut mac = Hmac::<Sha256>::new_from_slice(mac_key).expect("HMAC can take any size key");
        mac.update(&[VERSION]);
        mac.update(&iv);
        ...
        let body: Vec<u8> =
            std::iter::once(VERSION).chain(iv.iter().cloned()).chain(ciphertext).chain(mac).collect();
    ...
    pub fn decrypt(
        ...
        let master_ephemeral =
            PublicKey::deserialize(&provision_envelope.public_key.expect("no public key"))?;
        let body = provision_envelope.body.expect("no body in ProvisionMessage");
        if body[0] != VERSION {
            return Err(ProvisioningError::InvalidData { reason: "Bad version number".into() });
        }
        ...
        let mut verifier = Hmac::<Sha256>::new_from_slice(parts2).expect("HMAC can take any size key");
        verifier.update(iv_and_cipher_text);
        let our_mac = verifier.finalize().into_bytes();
        debug_assert_eq!(our_mac.len(), mac.len());
        if &our_mac[..32] != mac {
            return Err(ProvisioningError::InvalidData { reason: "wrong MAC".into() });
        }
        ...
        let input = cipher.decrypt_padded_vec_mut::<Pkcs7>(cipher_text).map_err(|e| {
            ProvisioningError::InvalidData { reason: format!("CBC/Padding error: {:?}", e).into() }
        })?;
...
mod tests {
    ...
    fn encrypt_provisioning_roundtrip() -> anyhow::Result<()> {
        ...
        let encrypt_cipher = ProvisioningCipher::from_public(*cipher.public_key());

        assert_eq!(cipher.public_key(), encrypt_cipher.public_key(), "copy public key");
