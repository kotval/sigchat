...
use crate::libsignal_protocol::utils::BASE64_RELAXED;
use crate::prelude::ServiceError;
pub use crate::proto::{ProvisionEnvelope, ProvisionMessage, ProvisioningVersion};
use crate::{
    account_manager::encrypt_device_name,
    pre_keys::{generate_last_resort_kyber_key, generate_signed_pre_key, PreKeysStore},
    push_service::{
        HttpAuth, LinkAccountAttributes, LinkCapabilities, LinkRequest, LinkResponse, PushService, ServiceIds,
    },
    utils::serde_base64,
};
...
pub fn generate_registration_id<R: rand::Rng + rand::CryptoRng>(csprng: &mut R) -> u32 {
...
pub fn link_device<R: rand::Rng + rand::CryptoRng, Aci: PreKeysStore, Pni: PreKeysStore, P: PushService>(
    aci_store: &mut Aci,
    pni_store: &mut Pni,
    csprng: &mut R,
    mut push_service: P,
    ...
    let ws = push_service.ws("/v1/websocket/provisioning/", "/v1/keepalive/provisioning", &[], None).await?;
    ...
    if let ProvisioningStep::Url(url) = provision_stream
        .next()
        .await
        .ok_or(ProvisioningError::InvalidData { reason: "no provisioning URL received".into() })??
    {
        tx.send(SecondaryDeviceProvisioning::Url(url))
            .await
            .expect("failed to send provisioning Url in channel");
    } else {
        return Err(ProvisioningError::InvalidData { reason: "wrong provisioning step received".into() });
    }

    if let ProvisioningStep::Message(message) = provision_stream
        .next()
        .await
        .ok_or(ProvisioningError::InvalidData { reason: "no provisioning message received".into() })??
    {
        let aci_public_key = PublicKey::deserialize(
            &message
                .aci_identity_key_public
                .ok_or(ProvisioningError::InvalidData { reason: "missing public key".into() })?,
        )?;
        ...
        let profile_key = message
            .profile_key
            .ok_or(ProvisioningError::InvalidData { reason: "missing profile key".into() })?;

        let phone_number =
            message.number.ok_or(ProvisioningError::InvalidData { reason: "missing phone number".into() })?;

        let phone_number = phonenumber::parse(None, phone_number).map_err(|e| {
            ProvisioningError::InvalidData { reason: format!("invalid phone number ({})", e).into() }
        })?;

        let provisioning_code = message
            .provisioning_code
            .ok_or(ProvisioningError::InvalidData { reason: "no provisioning confirmation code".into() })?;

        let aci_key_pair = KeyPair::new(aci_public_key, aci_private_key);
        let pni_key_pair = KeyPair::new(pni_public_key, pni_private_key);

        let aci_pq_last_resort_pre_key = generate_last_resort_kyber_key(aci_store, &aci_key_pair).await?;
        let pni_pq_last_resort_pre_key = generate_last_resort_kyber_key(pni_store, &pni_key_pair).await?;

        let aci_signed_pre_key = generate_signed_pre_key(aci_store, csprng, &aci_key_pair).await?;
        let pni_signed_pre_key = generate_signed_pre_key(pni_store, csprng, &pni_key_pair).await?;

        let encrypted_device_name =
            BASE64_RELAXED.encode(encrypt_device_name(csprng, device_name, &aci_public_key)?.encode_to_vec());

        let profile_key = ProfileKey::create(
            profile_key.as_slice().try_into().map_err(ProvisioningError::InvalidProfileKey)?,
        );
        ...
        let LinkResponse { aci, pni, device_id } = push_service
            .link_device(
                &request,
                HttpAuth { username: phone_number.to_string(), password: password.to_owned() },
            )
            .await?;
