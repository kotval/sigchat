...
// TODO: this base64 is not constant time which could lead to a side channel on cryptographic
// keys, but there are so many bigger issues before that even matters.
...
pub const BASE64_RELAXED: GeneralPurpose = GeneralPurpose::new(
...
pub mod serde_public_key {
        ...
        D: Deserializer<'de>,
    {
        PublicKey::deserialize(
            &BASE64_RELAXED.decode(String::deserialize(deserializer)?).map_err(serde::de::Error::custom)?,
        )
...
pub mod serde_optional_public_key {
    ...
    pub fn serialize<S>(public_key: &Option<PublicKey>, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        match public_key {
            Some(public_key) => serde_public_key::serialize(public_key, serializer),
            None => serializer.serialize_none(),
        }
    }
        ...
        D: Deserializer<'de>,
    {
        match Option::<String>::deserialize(deserializer)? {
            Some(public_key) => Ok(Some(
                PublicKey::deserialize(&BASE64_RELAXED.decode(public_key).map_err(serde::de::Error::custom)?)
                    .map_err(serde::de::Error::custom)?,
            )),
            None => Ok(None),
        }
...
pub mod serde_private_key {
    ...
    pub fn serialize<S>(public_key: &PrivateKey, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let public_key = public_key.serialize();
...
pub mod serde_optional_private_key {
    ...
    pub fn serialize<S>(private_key: &Option<PrivateKey>, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        match private_key {
            Some(private_key) => serde_private_key::serialize(private_key, serializer),
            None => serializer.serialize_none(),
        }
    }
        ...
        D: Deserializer<'de>,
    {
        match Option::<String>::deserialize(deserializer)? {
            Some(private_key) => Ok(Some(
                PrivateKey::deserialize(
                    &BASE64_RELAXED.decode(private_key).map_err(serde::de::Error::custom)?,
                )
                .map_err(serde::de::Error::custom)?,
            )),
            None => Ok(None),
...
pub mod serde_signaling_key {
    ...
    use super::BASE64_RELAXED;
    use crate::libsignal_service::envelope::SignalingKey;

    pub fn serialize<S>(signaling_key: &SignalingKey, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        ...
        D: Deserializer<'de>,
    {
        BASE64_RELAXED
            ...
            .try_into()
            .map_err(|buf: Vec<u8>| {
                serde::de::Error::invalid_length(buf.len(), &"invalid signaling key length")
            })
    }
}
...
pub mod serde_phone_number {
    ...
    where
        D: Deserializer<'de>,
    {
        phonenumber::parse(None, String::deserialize(deserializer)?).map_err(serde::de::Error::custom)
    }
}
